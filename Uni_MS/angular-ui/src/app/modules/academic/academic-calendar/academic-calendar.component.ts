  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage academic events and schedules</p>
      </div>
...
export class AcademicCalendarComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Event title' },
    { key: 'description', label: 'Description', type: 'textarea' },
    { key: 'eventType', label: 'Event Type', sortable: true, type: 'select', options: [{ label: 'Semester Start', value: 'Semester Start' }, { label: 'Semester End', value: 'Semester End' }, { label: 'Registration Deadline', value: 'Registration Deadline' }, { label: 'Exam Start', value: 'Exam Start' }, { label: 'Exam End', value: 'Exam End' }, { label: 'Holiday', value: 'Holiday' }, { label: 'Assignment Due', value: 'Assignment Due' }, { label: 'Other', value: 'Other' }], required: true },
    { key: 'startDate', label: 'Start Date', type: 'date', required: true },
    { key: 'endDate', label: 'End Date', type: 'date' },
    { key: 'semesterId', label: 'Semester ID', type: 'number', required: true, placeholder: 'Semester ID' },
    { key: 'isHoliday', label: 'Holiday', type: 'checkbox' },
  ...
  constructor(private service: AcademicCalendarService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load academic calendar'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Event updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Event created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Events';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected events? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} events deleted`);
            }
          },
