  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage academic sessions</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Academic Session' : 'Add New Academic Session'"
        [saving]="saving"
...
export class AcademicSessionsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: '2024-2025' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'S2425' },
  ...
  constructor(private service: AcademicSessionService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load academic sessions'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Academic session updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Academic session created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Academic Session';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Academic Sessions';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected academic sessions? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} academic sessions deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Academic session deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete academic session')
      });
    }
