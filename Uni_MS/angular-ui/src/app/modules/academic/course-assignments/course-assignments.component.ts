  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage course-subject assignments</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Course Assignment' : 'Add New Course Assignment'"
        [saving]="saving"
...
export class CourseAssignmentsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'administrationId', label: 'Administration ID', type: 'number', placeholder: 'Administration ID' },
    { key: 'semester', label: 'Semester', type: 'number', required: true, placeholder: 'Semester' }
  ];
  ...
  constructor(private service: CourseAssignmentService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load course assignments'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Course assignment updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Course assignment created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Course Assignment';
    this.confirmMessage = `Are you sure you want to delete assignment #${item.id}? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Course Assignments';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected course assignments? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} course assignments deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Course assignment deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete course assignment')
      });
    }
