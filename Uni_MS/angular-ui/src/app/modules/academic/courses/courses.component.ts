...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Courses</h2>
        <p class="page-sub">Manage academic courses</p>
      </div>
...
export class CoursesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Course Code', sortable: true },
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Enter course name' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'CRS001' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Describe the course' },
    { key: 'durationYears', label: 'Duration Years', type: 'number', required: true, placeholder: '4' },
  ...
  constructor(private service: CourseService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load courses'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Course updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Course created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Courses';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected courses? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} courses deleted`);
            }
          },
