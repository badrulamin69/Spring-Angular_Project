...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Credit Rules</h2>
        <p class="page-sub">Manage credit rules for programs</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Credit Rule' : 'Add New Credit Rule'"
        [saving]="saving"
...
export class CreditRulesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'minCreditsPerSemester', label: 'Min Credits/Sem', sortable: true, type: 'number', required: true, placeholder: '12' },
    { key: 'maxCreditsPerSemester', label: 'Max Credits/Sem', sortable: true, type: 'number', required: true, placeholder: '24' },
    { key: 'totalRequiredCredits', label: 'Total Required', sortable: true, type: 'number', required: true, placeholder: '140' },
    { key: 'maxTransferCredits', label: 'Max Transfer', type: 'number', placeholder: '0' },
    { key: 'maxElectiveCredits', label: 'Max Elective', type: 'number', placeholder: '0' },
  ...
  constructor(private service: CreditRuleService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load credit rules'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Credit rule updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Credit rule created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Credit Rule';
    this.confirmMessage = `Are you sure you want to delete this credit rule? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Credit Rules';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected credit rules? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} credit rules deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Credit rule deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete credit rule')
      });
    }
