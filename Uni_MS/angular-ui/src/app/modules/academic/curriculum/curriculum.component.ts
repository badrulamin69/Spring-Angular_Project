...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Curriculum</h2>
        <p class="page-sub">Manage curriculum mappings</p>
      </div>
...
export class CurriculumComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'orderNo', label: 'Order', type: 'number', placeholder: '1' },
  ...
  constructor(private service: CurriculumService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load curriculum'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Curriculum updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Curriculum created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Curriculum';
    this.confirmMessage = `Are you sure you want to delete this curriculum entry? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Curriculum';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected curriculum entries? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} curriculum entries deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Curriculum entry deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete curriculum entry')
      });
    }
