...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Faculties</h2>
        <p class="page-sub">Manage academic faculties and their departments</p>
      </div>
...
export class FacultiesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Faculty Code', sortable: true },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: '111' },
    { key: 'name', label: 'Faculty Name', sortable: true, type: 'text', required: true, placeholder: 'Faculty of Science' },
    { key: 'nameBn', label: 'Name (Bengali)', type: 'text', placeholder: 'αª¼αª┐αª£αºìαª₧αª╛αª¿ αªàαª¿αºüαª╖αªª' },
  ...
  constructor(private service: FacultyService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load faculties'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Faculty updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Faculty created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Faculty';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Faculties';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected faculties? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} faculties deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Faculty deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete faculty')
      });
    }
