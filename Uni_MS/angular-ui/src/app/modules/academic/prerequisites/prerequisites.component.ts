...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Prerequisites</h2>
        <p class="page-sub">Manage subject prerequisites</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Prerequisite' : 'Add New Prerequisite'"
        [saving]="saving"
...
export class PrerequisitesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'subjectId', label: 'Subject ID', sortable: true, type: 'number', required: true, placeholder: 'Subject ID' },
    { key: 'prerequisiteSubjectId', label: 'Prerequisite ID', sortable: true, type: 'number', required: true, placeholder: 'Prerequisite Subject ID' },
    { key: 'minGrade', label: 'Min Grade', type: 'select', options: [{ label: 'D', value: 'D' }, { label: 'C', value: 'C' }, { label: 'B', value: 'B' }, { label: 'A', value: 'A' }], required: true, placeholder: 'Select minimum grade' },
  ...
  constructor(private service: PrerequisiteService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load prerequisites'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Prerequisite updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Prerequisite created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Prerequisite';
    this.confirmMessage = `Are you sure you want to delete this prerequisite? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Prerequisites';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected prerequisites? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} prerequisites deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Prerequisite deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete prerequisite')
      });
    }
