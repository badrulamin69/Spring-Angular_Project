...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Programs</h2>
        <p class="page-sub">Manage academic programs</p>
      </div>
...
export class ProgramsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Program Code', sortable: true },
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'BSc in CSE' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'BSC-CSE' },
    { key: 'description', label: 'Description', type: 'textarea' },
    { key: 'programType', label: 'Type', type: 'select', required: true, options: [{ label: 'BSc', value: 'BSc' }, { label: 'MSc', value: 'MSc' }, { label: 'MBA', value: 'MBA' }, { label: 'PhD', value: 'PhD' }, { label: 'Diploma', value: 'Diploma' }, { label: 'Certificate', value: 'Certificate' }] },
    { key: 'durationYears', label: 'Duration', type: 'number', required: true, placeholder: '4' },
    { key: 'totalCredits', label: 'Total Credits', type: 'number', placeholder: '140' },
    { key: 'departmentId', label: 'Department ID', type: 'number', required: true, placeholder: 'Department ID' },
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load programs'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    const handleError = (err: any) => {
      this.saving = false;
      if (this.dynamicForm) {
        this.dynamicForm.parseValidationErrors(err);
      } else {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Program updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Program created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Program';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Programs';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected programs? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} programs deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Program deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete program')
      });
    }
