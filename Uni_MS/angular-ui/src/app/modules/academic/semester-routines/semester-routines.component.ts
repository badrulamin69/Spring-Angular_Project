...
import { SemesterRoutineService } from '../../../services/semester-routine.service';
...
@Component({
  selector: 'app-semester-routines',
  standalone: true,
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Semester Routines</h2>
        <p class="page-sub">Manage semester schedules and routines</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Semester Routine' : 'Add New Semester Routine'"
        [saving]="saving"
...
export class SemesterRoutinesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'batchId', label: 'Batch ID', type: 'number', required: true, placeholder: 'Batch ID' },
    { key: 'description', label: 'Description', type: 'textarea' },
    { key: 'totalWeeks', label: 'Total Weeks', type: 'number', placeholder: '16' },
    { key: 'midtermWeek', label: 'Midterm Week', type: 'number', placeholder: '8' },
    { key: 'finalExamWeek', label: 'Final Exam Week', type: 'number', placeholder: '16' },
  ...
  constructor(private service: SemesterRoutineService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load semester routines'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Semester routine updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Semester routine created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Semester Routine';
    this.confirmMessage = `Are you sure you want to delete this semester routine? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Semester Routines';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected semester routines? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} semester routines deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Semester routine deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete semester routine')
      });
    }
