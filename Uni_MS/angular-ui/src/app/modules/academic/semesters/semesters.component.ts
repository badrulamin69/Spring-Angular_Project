...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Semesters</h2>
        <p class="page-sub">Manage semesters</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Semester' : 'Add New Semester'"
        [saving]="saving"
...
export class SemestersComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Fall' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: '2024-FALL' },
    { key: 'academicSessionId', label: 'Session ID', type: 'number', required: true, placeholder: 'Session ID' },
    { key: 'orderNo', label: 'Order', type: 'number', required: true, placeholder: '1' },
    { key: 'startDate', label: 'Start', type: 'date', required: true },
    { key: 'endDate', label: 'End', type: 'date', required: true },
    { key: 'registrationDeadline', label: 'Reg Deadline', type: 'date' },
    { key: 'status', label: 'Status', type: 'select', sortable: true, required: true, options: [{ label: 'Upcoming', value: 'Upcoming' }, { label: 'Active', value: 'Active' }, { label: 'Completed', value: 'Completed' }] },
  ...
  constructor(private service: SemesterService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load semesters'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Semester updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Semester created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Semester';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Semesters';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected semesters? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} semesters deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete semester')
      });
    }
