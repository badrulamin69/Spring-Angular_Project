...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Subjects</h2>
        <p class="page-sub">Manage academic subjects</p>
      </div>
...
export class SubjectsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Enter subject name' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'SUB001' },
    { key: 'credits', label: 'Credits', type: 'number', required: true, placeholder: '3' },
  ...
  constructor(private service: SubjectService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load subjects'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Subject updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Subject created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Subjects';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected subjects? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} subjects deleted`);
            }
          },
