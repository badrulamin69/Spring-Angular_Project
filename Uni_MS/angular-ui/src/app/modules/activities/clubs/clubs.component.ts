...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Clubs</h2>
        <p class="page-sub">Manage student clubs and organizations</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Club' : 'Add New Club'"
        [saving]="saving"
...
export class ClubsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Enter club name' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'CLB001' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Describe the club' },
    { key: 'presidentName', label: 'President', type: 'text', placeholder: 'President name' },
    { key: 'advisorName', label: 'Advisor', type: 'text', placeholder: 'Advisor name' }
  ];
  ...
  constructor(private service: ClubService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load clubs'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Club updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Club created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Clubs';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected clubs? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} clubs deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Club deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete club')
      });
    }
