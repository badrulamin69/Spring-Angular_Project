  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage event registrations</p>
      </div>
...
export class RegistrationsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'eventId', label: 'Event ID', type: 'number', required: true, placeholder: 'Event ID' },
    ...
    { key: 'status', label: 'Status', type: 'text', placeholder: 'Pending/Confirmed' }
  ];
  ...
  constructor(private service: EventRegistrationService, private toastService: ToastService) {}
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Registration updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Registration created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Registration';
    this.confirmMessage = `Are you sure you want to delete registration #${item.id}? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Registrations';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected registrations? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} registrations deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Registration deleted successfully');
        },
