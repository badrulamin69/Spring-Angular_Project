...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Sports</h2>
        <p class="page-sub">Manage sports and athletic programs</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Sport' : 'Add New Sport'"
        [saving]="saving"
...
export class SportsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Enter sport name' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'SPT001' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Describe the sport' },
    { key: 'coachName', label: 'Coach', type: 'text', placeholder: 'Coach name' },
    { key: 'maxParticipants', label: 'Max Participants', type: 'number', placeholder: '50' }
  ];
  ...
  constructor(private service: SportService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load sports'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Sport updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Sport created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Sport';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Sports';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected sports? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} sports deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Sport deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete sport')
      });
    }
