...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Academic Heads</h2>
        <p class="page-sub">Heads of academic divisions and departments</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Academic Head' : 'Add New Academic Head'"
        [saving]="saving"
...
export class AcademicHeadsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'designation', label: 'Designation', type: 'text', required: true, placeholder: 'Dean / Chairman / Director' },
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Academic head updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Academic head created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Academic Head';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Academic Heads';
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Academic head deleted successfully');
        },
