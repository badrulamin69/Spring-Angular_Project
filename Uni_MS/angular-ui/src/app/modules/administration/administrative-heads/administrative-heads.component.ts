...
@Component({
  selector: 'app-administrative-heads',
  standalone: true,
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Administrative Heads</h2>
        <p class="page-sub">Senior administrative leadership of the university</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Administrative Head' : 'Add New Administrative Head'"
        [saving]="saving"
...
export class AdministrativeHeadsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Admin Head Code', sortable: true },
    ...
    { key: 'email', label: 'Email', sortable: true, type: 'email', required: true, placeholder: 'employee@university.edu' },
    { key: 'phone', label: 'Phone', type: 'text', placeholder: '+1 234 567 890' },
    { key: 'employeeCode', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'EMP001' },
    { key: 'designation', label: 'Designation', type: 'text', required: true, placeholder: 'Vice Chancellor' },
    { key: 'departmentId', label: 'Department ID', type: 'number', placeholder: 'Department ID' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'ACTIVE' }
  ];
  ...
  constructor(private service: EmployeeService, private toastService: ToastService) {}
  ...
  loadData() {
    this.loading = true;
    this.service.findAll(this.params, 'Chancellor OR Vice Chancellor OR Pro-Vice Chancellor OR Registrar OR Proctor').subscribe({
      next: (data) => { this.pagedData = data; this.loading = false; },
      error: () => { this.loading = false; this.toastService.error('Failed to load administrative heads'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Administrative head updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Administrative head created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Administrative Head';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Administrative Heads';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected records? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} records deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Administrative head deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete record')
      });
    }
