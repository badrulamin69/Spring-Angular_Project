  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Administrative office heads and officers</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Office Head' : 'Add New Office Head'"
        [saving]="saving"
...
export class HeadOfOfficesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'designation', label: 'Designation', type: 'text', required: true, placeholder: 'Registrar / Accountant' },
  ...
  loadData() {
    this.loading = true;
    this.service.findAll(this.params, 'Registrar OR Assistant Registrar OR Accountant OR Librarian OR Accounts Officer').subscribe({
      next: (data) => { this.pagedData = data; this.loading = false; },
      error: () => { this.loading = false; this.toastService.error('Failed to load office heads'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Office head updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Office head created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Office Head';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Office Heads';
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Office head deleted successfully');
        },
