...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Other Staff</h2>
        <p class="page-sub">General staff profiles and records</p>
      </div>
...
export class OthersComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'designation', label: 'Designation', type: 'text', required: true, placeholder: 'Staff designation' },
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load staff records'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Staff member updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Staff member created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Staff Member';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Staff Members';
