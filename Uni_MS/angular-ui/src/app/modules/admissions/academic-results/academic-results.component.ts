...
import { AcademicResultService } from '../../../services/academic-result.service';
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage admission academic results</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Academic Result' : 'Add New Academic Result'"
        [saving]="saving"
...
export class AcademicResultsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'institutionName', label: 'Institution', sortable: true, type: 'text', required: true, placeholder: 'Institution name' },
    { key: 'degree', label: 'Degree', type: 'text', placeholder: 'Degree' },
    { key: 'passingYear', label: 'Passing Year', type: 'number', placeholder: '2024' },
    { key: 'grade', label: 'Grade', type: 'text', placeholder: 'A+' },
    { key: 'percentage', label: 'Percentage', type: 'number', placeholder: '85.5' }
  ];
  ...
  constructor(private service: AcademicResultService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load academic results'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Academic result updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Academic result created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Academic Result';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Academic Results';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected academic results? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} academic results deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Academic result deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete academic result')
      });
    }
