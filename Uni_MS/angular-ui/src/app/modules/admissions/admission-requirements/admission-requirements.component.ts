...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div><h2>Admission Requirements</h2><p class="page-sub">Define and manage admission requirements per program</p></div>
    ...
    @if (showForm) { <app-dynamic-form [columns]="columns" [initialData]="editingItem" [title]="editingItem ? 'Edit Requirement' : 'Add New Requirement'" [saving]="saving" [errorMessage]="formError" (save)="saveItem($event)" (cancel)="closeForm()"></app-dynamic-form> }
...
export class AdmissionRequirementsComponent implements OnInit {
  ...
  private api = `${environment.apiUrl}/admission-requirements`;
  columns: TableColumn[] = [
    { key: 'id', label: 'ID', sortable: true },
    { key: 'programName', label: 'Program', sortable: true, type: 'text', required: true, placeholder: 'Program name' },
    { key: 'requirementType', label: 'Type', type: 'select', options: [{ label: 'Document', value: 'DOCUMENT' }, { label: 'Academic', value: 'ACADEMIC' }, { label: 'Test Score', value: 'TEST_SCORE' }, { label: 'Other', value: 'OTHER' }] },
    { key: 'description', label: 'Description', type: 'text', placeholder: 'Requirement description' },
    { key: 'isMandatory', label: 'Mandatory', type: 'select', options: [{ label: 'Yes', value: true }, { label: 'No', value: false }] },
  ...
  saveItem(data: any) {
    ...
    else { this.http.post(this.api, data).subscribe({ next: () => ok('Created'), error: err }); }
  }
  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Requirement'; this.confirmMessage = `Delete this requirement?`; this.showConfirm = true; }
  confirmBulkDelete(items: any[]) { this.deleteTarget = items; this.confirmTitle = 'Bulk Delete'; this.confirmMessage = `Delete ${items.length} requirements?`; this.showConfirm = true; }
