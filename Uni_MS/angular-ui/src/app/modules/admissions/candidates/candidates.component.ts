...
import { AdmissionCandidateService } from '../../../services/admission-candidate.service';
...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Admission Candidates</h2>
        <p class="page-sub">Manage admission candidate applications</p>
      </div>
...
export class CandidatesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'email', label: 'Email', sortable: true, type: 'email', required: true, placeholder: 'candidate@example.com' },
    { key: 'phone', label: 'Phone', type: 'text', placeholder: '+1 234 567 890' },
    { key: 'applicationNumber', label: 'Application No.', sortable: true, type: 'text', required: true, placeholder: 'APP001' },
    { key: 'status', label: 'Status', type: 'text', placeholder: 'Pending/Approved' },
    { key: 'appliedCourseId', label: 'Applied Course ID', type: 'number', placeholder: 'Course ID' }
  ];
  ...
  constructor(private service: AdmissionCandidateService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load candidates'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Candidate updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Candidate created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Candidate';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Candidates';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected candidates? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} candidates deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete candidate')
      });
    }
