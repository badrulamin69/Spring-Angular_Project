...
import { AdmissionCircularService } from '../../../services/admission-circular.service';
...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div><h2>Admission Circulars</h2><p class="page-sub">Manage admission circular announcements</p></div>
    ...
    @if (showForm) { <app-dynamic-form [columns]="columns" [initialData]="editingItem" [title]="editingItem ? 'Edit Circular' : 'Add New Circular'" [saving]="saving" [errorMessage]="formError" (save)="saveItem($event)" (cancel)="closeForm()"></app-dynamic-form> }
...
export class CircularsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Circular title' },
    { key: 'publishDate', label: 'Publish Date', type: 'date', required: true },
    { key: 'validUntil', label: 'Valid Until', type: 'date' },
    { key: 'isPublished', label: 'Published', type: 'select', options: [{ label: 'Yes', value: true }, { label: 'No', value: false }] },
  ...
  constructor(private service: AdmissionCircularService, private toastService: ToastService) {}
  ngOnInit() { this.loadData(); }
  loadData() { this.loading = true; this.service.findAll(this.params, this.searchTerm).subscribe({ next: (data) => { this.pagedData = data; this.loading = false; }, error: () => { this.loading = false; this.toastService.error('Failed to load circulars'); } }); }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) { this.service.update(this.editingItem.id, data).subscribe({ next: () => handleSuccess('Circular updated'), error: handleError }); }
    else { this.service.create(data).subscribe({ next: () => handleSuccess('Circular created'), error: handleError }); }
  }
  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Circular'; this.confirmMessage = `Delete "${item.title}"?`; this.showConfirm = true; }
  confirmBulkDelete(items: any[]) { this.deleteTarget = items; this.confirmTitle = 'Bulk Delete'; this.confirmMessage = `Delete ${items.length} circulars?`; this.showConfirm = true; }
  executeDelete() {
