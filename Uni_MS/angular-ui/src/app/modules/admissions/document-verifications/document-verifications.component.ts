...
@Component({
  selector: 'app-document-verifications',
  standalone: true,
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage document verification records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Verification' : 'Add New Verification'"
        [saving]="saving"
...
export class DocumentVerificationsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'documentType', label: 'Document Type', sortable: true, type: 'text', required: true, placeholder: 'Passport/ID' },
    { key: 'documentNumber', label: 'Document No.', type: 'text', placeholder: 'Document number' },
    { key: 'isVerified', label: 'Verified', type: 'checkbox' },
    { key: 'remarks', label: 'Remarks', type: 'textarea', placeholder: 'Verification remarks' }
  ];
  ...
  constructor(private service: DocumentVerificationService, private toastService: ToastService) {}
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Verification updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Verification created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmMessage = `Are you sure you want to delete verification #${item.id}? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Verifications';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected verifications? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} verifications deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Verification deleted successfully');
        },
