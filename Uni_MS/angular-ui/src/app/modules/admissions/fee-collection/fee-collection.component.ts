...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div><h2>Admission Fee Collection</h2><p class="page-sub">Track and manage admission-related fee payments</p></div>
      <button class="btn btn-primary" (click)="openForm()"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1v12M1 7h12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Record Payment</button>
    </div>
    ...
    @if (showForm) { <app-dynamic-form [columns]="columns" [initialData]="editingItem" [title]="editingItem ? 'Edit Payment' : 'Record New Payment'" [saving]="saving" [errorMessage]="formError" (save)="saveItem($event)" (cancel)="closeForm()"></app-dynamic-form> }
...
export class FeeCollectionComponent implements OnInit {
  ...
  private api = `${environment.apiUrl}/admission-fee-collection`;
  columns: TableColumn[] = [
    { key: 'id', label: 'ID', sortable: true },
    { key: 'candidateName', label: 'Candidate', sortable: true, type: 'text', required: true, placeholder: 'Candidate name' },
    { key: 'applicationNumber', label: 'Application No.', sortable: true, type: 'text', placeholder: 'APP001' },
    { key: 'feeType', label: 'Fee Type', type: 'select', options: [{ label: 'Application Fee', value: 'APPLICATION' }, { label: 'Registration Fee', value: 'REGISTRATION' }, { label: 'Tuition Fee', value: 'TUITION' }, { label: 'Lab Fee', value: 'LAB' }, { label: 'Other', value: 'OTHER' }] },
    ...
    { key: 'paymentMethod', label: 'Method', type: 'select', options: [{ label: 'Cash', value: 'CASH' }, { label: 'Bank Transfer', value: 'BANK_TRANSFER' }, { label: 'Card', value: 'CARD' }, { label: 'Online', value: 'ONLINE' }] },
    { key: 'status', label: 'Status', type: 'select', options: [{ label: 'Pending', value: 'PENDING' }, { label: 'Paid', value: 'PAID' }, { label: 'Refunded', value: 'REFUNDED' }, { label: 'Failed', value: 'FAILED' }] }
  ];
  ...
  saveItem(data: any) {
    ...
    else { this.http.post(this.api, data).subscribe({ next: () => ok('Payment recorded'), error: err }); }
  }
  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Payment'; this.confirmMessage = `Delete this payment record?`; this.showConfirm = true; }
  confirmBulkDelete(items: any[]) { this.deleteTarget = items; this.confirmTitle = 'Bulk Delete'; this.confirmMessage = `Delete ${items.length} payments?`; this.showConfirm = true; }
  executeDelete() { this.showConfirm = false; if (Array.isArray(this.deleteTarget)) { let c = 0; this.deleteTarget.forEach((i: any) => { this.http.delete(`${this.api}/${i.id}`).subscribe({ next: () => { c++; if (c === this.deleteTarget.length) { this.loadData(); this.toastService.success('Deleted'); } } }); }); } else if (this.deleteTarget?.id) { this.http.delete(`${this.api}/${this.deleteTarget.id}`).subscribe({ next: () => { this.loadData(); this.toastService.success('Deleted'); } }); } this.deleteTarget = null; }
}
