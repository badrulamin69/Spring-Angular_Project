...
@Component({
  ...
  template: `
    <div class="page-header">
      <div>
        <h2>Seat Plan</h2>
        <p class="page-sub">Manage seat allocations for admission tests</p>
      </div>
      <div class="header-actions">
        <button class="btn btn-outline" (click)="autoGenerate()" [disabled]="!selectedTestId || generating">
...
export class SeatPlanComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'seatNumber', label: 'Seat No', sortable: true },
  ...
  loadData() {
    this.loading = true;
    this.seatService.findAll(this.params, '', this.selectedTestId || undefined).subscribe({
      next: (data) => { this.pagedData = data; this.loading = false; },
      error: () => { this.loading = false; this.toast.error('Failed to load seat allocations'); }
    });
  }
  ...
  autoGenerate() {
    ...
    this.seatService.autoGenerate(this.selectedTestId).subscribe({
      next: (res) => { this.generating = false; this.toast.success(res.message || 'Seats generated'); this.loadData(); },
      error: (err) => { this.generating = false; this.toast.error(err.error?.message || 'Failed to generate seats'); }
    });
  }
}
