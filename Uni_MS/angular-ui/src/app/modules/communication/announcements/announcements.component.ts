...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Announcements</h2>
        <p class="page-sub">Manage announcements and posts</p>
      </div>
...
export class AnnouncementsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Announcement title' },
    { key: 'content', label: 'Content', type: 'textarea', required: true, placeholder: 'Announcement content' },
    { key: 'type', label: 'Type', type: 'text', placeholder: 'General/Academic' },
    { key: 'postedById', label: 'Posted By', type: 'number', placeholder: 'User ID' }
  ];
  ...
  constructor(private service: AnnouncementService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load announcements'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Announcement updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Announcement created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Announcement';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Announcements';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected announcements? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} announcements deleted`);
            }
          },
