...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Messages</h2>
        <p class="page-sub">Manage internal messages</p>
      </div>
...
export class MessagesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'senderId', label: 'Sender ID', type: 'number', required: true, placeholder: 'Sender ID' },
    { key: 'receiverId', label: 'Receiver ID', type: 'number', required: true, placeholder: 'Receiver ID' },
    { key: 'subject', label: 'Subject', sortable: true, type: 'text', placeholder: 'Message subject' },
    { key: 'body', label: 'Body', type: 'textarea', required: true, placeholder: 'Message body' }
  ];
  ...
  constructor(private service: MessageService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load messages'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Message updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Message created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Messages';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected messages? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} messages deleted`);
            }
          },
