...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Notices</h2>
        <p class="page-sub">Manage notices and announcements</p>
      </div>
...
export class NoticesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Notice title' },
    { key: 'content', label: 'Content', type: 'textarea', required: true, placeholder: 'Notice content' },
    { key: 'priority', label: 'Priority', type: 'text', placeholder: 'High/Medium/Low' },
    { key: 'targetAudience', label: 'Audience', type: 'text', placeholder: 'All Students' },
  ...
  constructor(private service: NoticeService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load notices'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Notice updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Notice created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Notice';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Notices';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected notices? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} notices deleted`);
            }
          },
