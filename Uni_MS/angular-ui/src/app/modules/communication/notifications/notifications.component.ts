...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Notifications</h2>
        <p class="page-sub">Manage user notifications</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Notification' : 'Add New Notification'"
        [saving]="saving"
...
export class NotificationsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'userId', label: 'User ID', type: 'number', required: true, placeholder: 'User ID' },
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Notification title' },
    { key: 'message', label: 'Message', type: 'textarea', required: true, placeholder: 'Notification message' },
    { key: 'type', label: 'Type', type: 'text', placeholder: 'Info/Warning/Error' },
  ...
  constructor(private service: NotificationService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load notifications'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Notification updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Notification created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Notifications';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected notifications? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} notifications deleted`);
            }
          },
