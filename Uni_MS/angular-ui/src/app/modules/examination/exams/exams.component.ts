...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Exams</h2>
        <p class="page-sub">Manage examinations and tests</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Exam' : 'Add New Exam'"
        [saving]="saving"
...
export class ExamsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Exam name' },
    { key: 'examType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Midterm/Final' },
    ...
    { key: 'totalMarks', label: 'Total Marks', sortable: true, type: 'number', required: true, placeholder: '100' },
    { key: 'passingMarks', label: 'Passing Marks', type: 'number', required: true, placeholder: '40' },
  ...
  constructor(private service: ExamService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load exams'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Exam updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Exam created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Exams';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected exams? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} exams deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete exam')
      });
    }
