...
import { GradeRuleService } from '../../../services/grade-rule.service';
...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Grade Rules</h2>
        <p class="page-sub">Manage grading rules and scales</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Grade Rule' : 'Add New Grade Rule'"
        [saving]="saving"
...
export class GradeRulesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'grade', label: 'Grade', sortable: true, type: 'text', required: true, placeholder: 'A+' },
    { key: 'minPercentage', label: 'Min %', sortable: true, type: 'number', required: true, placeholder: '90' },
    { key: 'maxPercentage', label: 'Max %', type: 'number', required: true, placeholder: '100' },
    { key: 'gradePoint', label: 'Grade Point', type: 'number', required: true, placeholder: '4.0' },
  ...
  constructor(private service: GradeRuleService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load grade rules'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Grade rule updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Grade rule created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Grade Rule';
    this.confirmMessage = `Are you sure you want to delete this grade rule? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Grade Rules';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected grade rules? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} grade rules deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Grade rule deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete grade rule')
      });
    }
