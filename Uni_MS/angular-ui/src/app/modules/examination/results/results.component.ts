...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Exam Results</h2>
        <p class="page-sub">Manage exam results and grades</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Exam Result' : 'Add New Exam Result'"
        [saving]="saving"
...
export class ExamResultsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'examId', label: 'Exam ID', type: 'number', required: true, placeholder: 'Exam ID' },
    { key: 'studentId', label: 'Student ID', type: 'number', required: true, placeholder: 'Student ID' },
    { key: 'totalMarksObtained', label: 'Marks Obtained', sortable: true, type: 'number', required: true, placeholder: '85' },
    { key: 'totalMarks', label: 'Total Marks', type: 'number', required: true, placeholder: '100' },
    { key: 'percentage', label: 'Percentage', sortable: true, type: 'number', placeholder: '85.0' },
    { key: 'grade', label: 'Grade', type: 'text', placeholder: 'A' },
    { key: 'resultStatus', label: 'Status', sortable: true, type: 'text', placeholder: 'Pass/Fail' }
  ];
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load exam results'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Exam result updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Exam result created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Exam Result';
    this.confirmMessage = `Are you sure you want to delete this exam result? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Exam Results';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected exam results? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} exam results deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Exam result deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete exam result')
      });
    }
