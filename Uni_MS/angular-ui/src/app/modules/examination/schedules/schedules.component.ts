  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage exam schedules and timetables</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Exam Schedule' : 'Add New Exam Schedule'"
        [saving]="saving"
...
export class SchedulesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'endTime', label: 'End Time', type: 'text', required: true, placeholder: '12:00' },
    { key: 'venue', label: 'Venue', type: 'text', placeholder: 'Hall A' },
  ...
  constructor(private service: ExamScheduleService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load exam schedules'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Exam schedule updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Exam schedule created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Exam Schedule';
    this.confirmMessage = `Are you sure you want to delete this exam schedule? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Exam Schedules';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected exam schedules? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} exam schedules deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Exam schedule deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete exam schedule')
      });
    }
