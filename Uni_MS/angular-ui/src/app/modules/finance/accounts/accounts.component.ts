...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Accounts</h2>
        <p class="page-sub">Manage financial accounts</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Account' : 'Add New Account'"
        [saving]="saving"
...
export class AccountsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'accountName', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Account name' },
    { key: 'accountNumber', label: 'Number', sortable: true, type: 'text', required: true, placeholder: 'ACC001' },
    { key: 'accountType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Savings/Current' },
    { key: 'balance', label: 'Balance', sortable: true, type: 'number', placeholder: '0.00' },
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load accounts'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Account updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Account created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Account';
    this.confirmMessage = `Are you sure you want to delete "${item.accountName}"? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Accounts';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected accounts? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} accounts deleted`);
            }
          },
