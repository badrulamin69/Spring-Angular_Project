...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Discounts</h2>
        <p class="page-sub">Manage student discounts</p>
      </div>
...
export class DiscountsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'discountType', label: 'Type', sortable: true, type: 'text' },
    { key: 'discountValue', label: 'Value', sortable: true, type: 'number' },
  ...
  formColumns: TableColumn[] = [
    ...
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Discount description' },
    { key: 'validFrom', label: 'Valid From', type: 'date' },
    { key: 'validTo', label: 'Valid To', type: 'date' },
  ...
  constructor(private service: DiscountService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load discounts'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Discount updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Discount created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Discount';
    this.confirmMessage = `Are you sure you want to delete this discount? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Discounts';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected discounts? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} discounts deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Discount deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete discount')
      });
    }
