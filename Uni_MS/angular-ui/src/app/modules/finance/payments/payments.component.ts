...
export class PaymentsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'paymentDate', label: 'Date', type: 'date', required: true },
    { key: 'paymentMethod', label: 'Method', type: 'text', placeholder: 'Cash/Card/Bank' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Completed/Pending' }
  ];
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Payment updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Payment created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmMessage = `Are you sure you want to delete this payment? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Payments';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected payments? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} payments deleted`);
            }
          },
