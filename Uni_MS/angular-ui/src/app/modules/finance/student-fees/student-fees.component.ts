...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Student Fees</h2>
        <p class="page-sub">Manage student fee records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Student Fee' : 'Add New Student Fee'"
        [saving]="saving"
...
export class StudentFeesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'feeTypeId', label: 'Fee Type ID', type: 'number', required: true, placeholder: 'Fee Type ID' },
    { key: 'amount', label: 'Amount', sortable: true, type: 'number', required: true, placeholder: '5000.00' },
    { key: 'dueDate', label: 'Due Date', type: 'date', required: true },
    { key: 'paidAmount', label: 'Paid', type: 'number', placeholder: '0.00' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Pending/Paid' },
    { key: 'academicYear', label: 'Year', type: 'text', placeholder: '2024-2025' }
  ];
  ...
  constructor(private service: StudentFeeService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load student fees'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Student fee updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Student fee created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Student Fee';
    this.confirmMessage = `Are you sure you want to delete this student fee? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Student Fees';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected student fees? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} student fees deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Student fee deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete student fee')
      });
    }
