...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Transactions</h2>
        <p class="page-sub">Manage financial transactions</p>
      </div>
...
export class TransactionsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'accountId', label: 'Account ID', type: 'number', required: true, placeholder: 'Account ID' },
    { key: 'transactionType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Credit/Debit' },
    { key: 'amount', label: 'Amount', sortable: true, type: 'number', required: true, placeholder: '5000.00' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Transaction description' },
  ...
  constructor(private service: TransactionService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load transactions'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Transaction updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Transaction created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmMessage = `Are you sure you want to delete this transaction? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Transactions';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected transactions? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} transactions deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete transaction')
      });
    }
