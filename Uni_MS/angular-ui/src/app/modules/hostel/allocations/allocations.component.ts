...
import { HostelAllocationService } from '../../../services/hostel-allocation.service';
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage hostel allocation records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Hostel Allocation' : 'Add New Hostel Allocation'"
        [saving]="saving"
...
export class HostelAllocationsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'roomId', label: 'Room ID', type: 'number', required: true, placeholder: 'Room ID' },
    { key: 'allocationDate', label: 'Allocated', type: 'date', required: true },
    { key: 'endDate', label: 'End Date', type: 'date' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Active/Inactive' },
    { key: 'monthlyRent', label: 'Rent', type: 'number', placeholder: '5000' }
  ];
  ...
  constructor(private service: HostelAllocationService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load hostel allocations'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Hostel allocation updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Hostel allocation created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Hostel Allocation';
    this.confirmMessage = `Are you sure you want to delete this hostel allocation? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Hostel Allocations';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected hostel allocations? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} hostel allocations deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Hostel allocation deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete hostel allocation')
      });
    }
