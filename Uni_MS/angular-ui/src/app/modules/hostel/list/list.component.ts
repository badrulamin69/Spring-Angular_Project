...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Hostels</h2>
        <p class="page-sub">Manage hostel records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Hostel' : 'Add New Hostel'"
        [saving]="saving"
...
export class HostelsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Hostel name' },
    { key: 'code', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'HOS001' },
    { key: 'type', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Male/Female' },
    { key: 'address', label: 'Address', type: 'textarea', placeholder: 'Hostel address' },
    { key: 'wardensName', label: 'Warden', type: 'text', placeholder: 'Warden name' },
    { key: 'totalRooms', label: 'Total Rooms', sortable: true, type: 'number', required: true, placeholder: '100' }
  ];
  ...
  constructor(private service: HostelService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load hostels'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Hostel updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Hostel created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Hostel';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Hostels';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected hostels? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} hostels deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete hostel')
      });
    }
