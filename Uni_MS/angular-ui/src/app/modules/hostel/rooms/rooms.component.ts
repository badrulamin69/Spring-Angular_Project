...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Rooms</h2>
        <p class="page-sub">Manage hostel rooms</p>
      </div>
...
export class RoomsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'roomNumber', label: 'Room No.', sortable: true, type: 'text', required: true, placeholder: '101' },
    { key: 'floor', label: 'Floor', type: 'number', placeholder: '1' },
    { key: 'capacity', label: 'Capacity', sortable: true, type: 'number', required: true, placeholder: '4' },
    { key: 'currentOccupancy', label: 'Occupied', type: 'number', placeholder: '0' },
    { key: 'roomType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Single/Double' },
    { key: 'monthlyRent', label: 'Rent', sortable: true, type: 'number', placeholder: '5000' },
    { key: 'hostelId', label: 'Hostel ID', type: 'number', required: true, placeholder: 'Hostel ID' }
  ];
  ...
  constructor(private service: RoomService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load rooms'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Room updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Room created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Room';
    this.confirmMessage = `Are you sure you want to delete room "${item.roomNumber}"? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Rooms';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected rooms? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} rooms deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete room')
      });
    }
