  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage employee attendance records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Attendance' : 'Add New Attendance'"
        [saving]="saving"
...
export class EmployeeAttendanceComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'status', label: 'Status', sortable: true, type: 'text', required: true, placeholder: 'Present/Absent' },
    { key: 'checkIn', label: 'Check In', type: 'text', placeholder: '09:00' },
    { key: 'checkOut', label: 'Check Out', type: 'text', placeholder: '17:00' },
    { key: 'remarks', label: 'Remarks', type: 'textarea', placeholder: 'Remarks' }
  ];
  ...
  constructor(private service: EmployeeAttendanceService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load attendance records'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Attendance updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Attendance created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Attendance';
    this.confirmMessage = `Are you sure you want to delete this attendance record? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Attendance';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected attendance records? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} attendance records deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete attendance record')
      });
    }
