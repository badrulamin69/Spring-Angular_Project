...
export class EmployeesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Employee Code', sortable: true },
    ...
    { key: 'designation', label: 'Designation', type: 'text', placeholder: 'Accountant/ Clerk' },
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load employees'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Employee updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Employee created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Employees';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected employees? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} employees deleted`);
            }
          },
