  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage employee leave requests</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Leave Request' : 'Add New Leave Request'"
        [saving]="saving"
...
export class LeaveRequestsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'leaveType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Sick/Casual/Annual' },
    { key: 'startDate', label: 'Start', type: 'date', required: true },
    { key: 'endDate', label: 'End', type: 'date', required: true },
    { key: 'reason', label: 'Reason', type: 'textarea', required: true, placeholder: 'Reason for leave' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Pending/Approved/Rejected' }
  ];
  ...
  constructor(private service: LeaveRequestService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load leave requests'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Leave request updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Leave request created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmMessage = `Are you sure you want to delete this leave request? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Leave Requests';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected leave requests? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} leave requests deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete leave request')
      });
    }
