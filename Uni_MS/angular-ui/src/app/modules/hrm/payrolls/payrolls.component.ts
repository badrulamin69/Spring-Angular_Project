...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Payrolls</h2>
        <p class="page-sub">Manage payroll records</p>
      </div>
...
export class PayrollsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'employeeId', label: 'Employee ID', type: 'number', required: true, placeholder: 'Employee ID' },
    { key: 'payPeriodStart', label: 'Period Start', type: 'date', required: true },
    { key: 'payPeriodEnd', label: 'Period End', type: 'date', required: true },
    { key: 'basicSalary', label: 'Salary', sortable: true, type: 'number', required: true, placeholder: '50000' },
    { key: 'allowances', label: 'Allowances', type: 'number', placeholder: '5000' },
    { key: 'deductions', label: 'Deductions', type: 'number', placeholder: '3000' },
    { key: 'netSalary', label: 'Net Salary', sortable: true, type: 'number', required: true, placeholder: '52000' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Pending/Paid' }
  ];
  ...
  constructor(private service: PayrollService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load payrolls'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Payroll updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Payroll created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Payroll';
    this.confirmMessage = `Are you sure you want to delete this payroll record? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Payrolls';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected payrolls? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} payrolls deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Payroll deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete payroll')
      });
    }
