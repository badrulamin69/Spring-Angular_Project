...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Books</h2>
        <p class="page-sub">Manage library books</p>
      </div>
...
export class BooksComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Book Code', sortable: true },
    { key: 'categoryId', label: 'Category ID', type: 'number', placeholder: 'Category ID' },
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Book title' },
    { key: 'author', label: 'Author', sortable: true, type: 'text', required: true, placeholder: 'Author name' },
    { key: 'isbn', label: 'ISBN', sortable: true, type: 'text', placeholder: '978-0-123456-78-9' },
    { key: 'publisher', label: 'Publisher', type: 'text', placeholder: 'Publisher name' },
    { key: 'totalCopies', label: 'Total Copies', sortable: true, type: 'number', required: true, placeholder: '10' },
    { key: 'availableCopies', label: 'Available', sortable: true, type: 'number', required: true, placeholder: '8' },
    { key: 'shelfLocation', label: 'Shelf', type: 'text', placeholder: 'A-1-01' }
  ];
  ...
  constructor(private service: BookService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load books'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Book updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Book created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Books';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected books? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} books deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete book')
      });
    }
