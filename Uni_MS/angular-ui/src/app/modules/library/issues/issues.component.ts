...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Book Issues</h2>
        <p class="page-sub">Manage book issue records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Book Issue' : 'Add New Book Issue'"
        [saving]="saving"
...
export class IssuesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'bookId', label: 'Book ID', type: 'number', required: true, placeholder: 'Book ID' },
    ...
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Issued/Returned' }
  ];
  ...
  constructor(private service: BookIssueService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load book issues'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Book issue updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Book issue created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Book Issue';
    this.confirmMessage = `Are you sure you want to delete this book issue? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Book Issues';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected book issues? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} book issues deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Book issue deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete book issue')
      });
    }
