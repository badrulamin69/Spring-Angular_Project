...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Book Returns</h2>
        <p class="page-sub">Manage book return records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Book Return' : 'Add New Book Return'"
        [saving]="saving"
...
export class ReturnsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'bookIssueId', label: 'Issue ID', type: 'number', required: true, placeholder: 'Issue ID' },
    { key: 'returnDate', label: 'Return Date', type: 'date', required: true },
    { key: 'fineAmount', label: 'Fine', sortable: true, type: 'number', placeholder: '0.00' },
    { key: 'finePaid', label: 'Fine Paid', type: 'checkbox' },
    { key: 'conditionAtReturn', label: 'Condition', type: 'text', placeholder: 'Good/Damaged' },
  ...
  constructor(private service: BookReturnService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load book returns'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Book return updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Book return created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Book Return';
    this.confirmMessage = `Are you sure you want to delete this book return? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Book Returns';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected book returns? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} book returns deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Book return deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete book return')
      });
    }
