...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Assignments</h2>
        <p class="page-sub">Manage assignment records</p>
      </div>
...
export class LmsAssignmentsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Assignment title' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Description' },
    { key: 'dueDate', label: 'Due Date', type: 'date', required: true },
    { key: 'maxMarks', label: 'Max Marks', sortable: true, type: 'number', required: true, placeholder: '100' },
  ...
  constructor(private service: AssignmentService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load assignments'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Assignment updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Assignment created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Assignment';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Assignments';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected assignments? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} assignments deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete assignment')
      });
    }
