  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage course materials</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Course Material' : 'Add New Course Material'"
        [saving]="saving"
...
export class MaterialsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Material title' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Description' },
    { key: 'materialType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'PDF/Video/Slide' },
  ...
  constructor(private service: CourseMaterialService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load course materials'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Course material updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Course material created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Course Material';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Course Materials';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected course materials? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} course materials deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Course material deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete course material')
      });
    }
