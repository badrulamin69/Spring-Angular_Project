  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage online class sessions</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Online Class' : 'Add New Online Class'"
        [saving]="saving"
...
export class OnlineClassesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Class title' },
    { key: 'description', label: 'Description', type: 'textarea', placeholder: 'Description' },
    { key: 'meetingUrl', label: 'Meeting URL', type: 'text', required: true, placeholder: 'https://meet.example.com' },
    { key: 'classDate', label: 'Date', type: 'date', required: true },
    { key: 'startTime', label: 'Start', type: 'text', required: true, placeholder: '10:00' },
    { key: 'endTime', label: 'End', type: 'text', required: true, placeholder: '12:00' },
    { key: 'courseId', label: 'Course ID', type: 'number', required: true, placeholder: 'Course ID' },
  ...
  constructor(private service: OnlineClassService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load online classes'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Online class updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Online class created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Online Class';
    this.confirmMessage = `Are you sure you want to delete "${item.title}"? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Online Classes';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected online classes? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} online classes deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Online class deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete online class')
      });
    }
