...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Submissions</h2>
        <p class="page-sub">Manage assignment submissions</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Submission' : 'Add New Submission'"
        [saving]="saving"
...
export class SubmissionsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'assignmentId', label: 'Assignment ID', type: 'number', required: true, placeholder: 'Assignment ID' },
    { key: 'studentId', label: 'Student ID', type: 'number', required: true, placeholder: 'Student ID' },
    { key: 'submissionDate', label: 'Submitted', type: 'date', required: true },
    { key: 'marksObtained', label: 'Marks', sortable: true, type: 'number', placeholder: '85' },
    { key: 'feedback', label: 'Feedback', type: 'textarea', placeholder: 'Feedback' },
    { key: 'status', label: 'Status', sortable: true, type: 'text', placeholder: 'Submitted/Graded' }
  ];
  ...
  constructor(private service: AssignmentSubmissionService, private toastService: ToastService) {}
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Submission updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Submission created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmMessage = `Are you sure you want to delete this submission? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Submissions';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected submissions? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} submissions deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete submission')
      });
    }
