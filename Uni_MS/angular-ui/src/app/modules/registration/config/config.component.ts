  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Configure course registration periods and rules</p>
      </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            @for (config of configs; track config.id) {
                ...
                <td>
                  <div class="actions">
                    ...
                    @if (!config.isClosed && config.isActive) {
                      <button class="btn-icon btn-warning" (click)="closeConfig(config)" title="Close Registration">
              ...
              <tr><td colspan="8" class="text-center text-muted">No registration configurations found</td></tr>
            }
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-overlay" (click)="closeForm()">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h3>{{ editingItem ? 'Edit' : 'New' }} Registration Config</h3>
          ...
          <form (ngSubmit)="save()">
            <div class="form-grid">
              ...
              <div class="form-group">
                ...
                <input type="date" [(ngModel)]="form.addDropDeadline" name="addDropDeadline" class="form-control">
              </div>
              <div class="form-group checkbox-group">
                <label>
                  <input type="checkbox" [(ngModel)]="form.allowAddDrop" name="allowAddDrop"> Allow Add/Drop
                </label>
              </div>
              <div class="form-group checkbox-group">
                <label>
                  <input type="checkbox" [(ngModel)]="form.advisorApprovalRequired" name="advisorApprovalRequired"> Advisor Approval Required
                </label>
              </div>
              <div class="form-group checkbox-group">
                <label>
                  <input type="checkbox" [(ngModel)]="form.paymentRequired" name="paymentRequired"> Payment Required
                </label>
              </div>
              <div class="form-group">
                <label>Remarks</label>
...
export class RegistrationConfigComponent implements OnInit {
  ...
  form: Partial<RegistrationConfig> = {
    semesterId: 0, startDate: '', endDate: '', minCredits: 12, maxCredits: 24,
    allowAddDrop: true, advisorApprovalRequired: true, paymentRequired: true, isActive: true
  };

  constructor(
    private configService: RegistrationConfigService,
  ...
  save() {
    ...
    const handleError = (err: any) => { this.saving = false; this.formError = err.error?.message || err.error?.data || 'Save failed'; };

    if (this.editingItem?.id) {
      this.configService.update(this.editingItem.id, this.form as RegistrationConfig).subscribe({ next: () => handleSuccess('Config updated'), error: handleError });
    } else {
      this.configService.create(this.form as RegistrationConfig).subscribe({ next: () => handleSuccess('Config created'), error: handleError });
    }
  }

  closeConfig(config: RegistrationConfig) {
    if (confirm('Close registration for this semester?')) {
      this.configService.closeRegistration(config.id!).subscribe({
        next: () => { this.loadData(); this.toastService.success('Registration closed'); },
        error: () => this.toastService.error('Failed to close')
      });
    }
  }

  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Config'; this.confirmMessage = 'Are you sure you want to delete this configuration?'; this.showConfirm = true; }
