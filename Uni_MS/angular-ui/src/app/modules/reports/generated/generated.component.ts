  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">View and manage generated reports</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Generated Report' : 'Add New Generated Report'"
        [saving]="saving"
...
export class GeneratedComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'title', label: 'Title', sortable: true, type: 'text', required: true, placeholder: 'Report title' },
    { key: 'reportType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Academic/Finance' },
    { key: 'format', label: 'Format', type: 'text', placeholder: 'PDF/Excel' },
    { key: 'generatedAt', label: 'Generated', type: 'date' },
    { key: 'fileUrl', label: 'File URL', type: 'text', placeholder: 'Download URL' },
    { key: 'templateId', label: 'Template ID', type: 'number', placeholder: 'Template ID' }
  ];
  ...
  constructor(private service: GeneratedReportService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load generated reports'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Generated report updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Generated report created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Generated Report';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Generated Reports';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected generated reports? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} generated reports deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Generated report deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete generated report')
      });
    }
