...
import { AcademicCalendar } from '../../../models/class-routine';
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">View and manage academic calendar entries</p>
      </div>
    ...
    <div class="filters-bar">
      ...
      <div class="search-box">
        ...
        <input type="text" [(ngModel)]="searchTerm" placeholder="Search calendar entries..." (input)="filterData()">
      </div>
    </div>
          ...
          <tbody>
            @for (entry of filteredEntries; track entry.id) {
              <tr>
                ...
                <td><span class="badge badge-info">{{ entry.eventType }}</span></td>
                <td>{{ entry.startDate }}</td>
                <td>{{ entry.endDate || '-' }}</td>
                <td>{{ entry.semesterName || ('Semester #' + entry.semesterId) }}</td>
                <td>
                  @if (entry.isHoliday) {
                ...
                <td>
                  <div class="actions">
                    <button class="btn-icon" (click)="openForm(entry)" title="Edit">
              ...
              <tr><td colspan="9" class="text-center text-muted">No academic calendar entries found</td></tr>
            }
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-overlay" (click)="closeForm()">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h3>{{ editingItem ? 'Edit' : 'New' }} Calendar Entry</h3>
          ...
          <form (ngSubmit)="save()">
            <div class="form-grid">
              <div class="form-group full-width">
                <label>Title *</label>
                <input type="text" [(ngModel)]="form.title" name="title" required class="form-control">
              </div>
              <div class="form-group full-width">
...
export class AcademicCalendarComponent implements OnInit {
  entries: AcademicCalendar[] = [];
  filteredEntries: AcademicCalendar[] = [];
  semesters: any[] = [];
  loading = true;
  saving = false;
  showForm = false;
  editingItem: AcademicCalendar | null = null;
  formError = '';
  showConfirm = false;
  confirmTitle = '';
  confirmMessage = '';
  deleteTarget: AcademicCalendar | null = null;
  searchTerm = '';
  filterSemester = '';

  form: Partial<AcademicCalendar> = {
    title: '', description: '', eventType: '', startDate: '', endDate: '',
    semesterId: 0, isHoliday: false, isPublished: false, color: '#3b82f6'
  };
    ...
    this.service.getCalendarEvents().subscribe({
      next: (data) => {
        this.entries = data.map(e => ({
          ...
          startDate: e.startDate, endDate: e.endDate, semesterId: e.semesterId,
          semesterName: e.semesterName, isHoliday: e.isHoliday, isPublished: e.isPublished,
          color: e.color, createdAt: e.createdAt
        }));
        this.filterData();
        this.loading = false;
      },
      error: () => { this.loading = false; this.toastService.error('Failed to load calendar entries'); }
    });
  }
  ...
  openForm(item?: AcademicCalendar) {
  ...
  save() {
    ...
    if (this.editingItem?.id) {
      this.service.updateCalendarEvent(this.editingItem.id, this.form as any).subscribe({ next: () => handleSuccess('Entry updated'), error: handleError });
    } else {
      this.service.createCalendarEvent(this.form as any).subscribe({ next: () => handleSuccess('Entry created'), error: handleError });
    }
  }

  confirmDelete(item: AcademicCalendar) {
    this.deleteTarget = item;
    this.confirmTitle = 'Delete Calendar Entry';
  ...
  executeDelete() {
    ...
    if (this.deleteTarget?.id) {
      this.service.deleteCalendarEvent(this.deleteTarget.id).subscribe({
        next: () => { this.loadData(); this.toastService.success('Entry deleted'); },
