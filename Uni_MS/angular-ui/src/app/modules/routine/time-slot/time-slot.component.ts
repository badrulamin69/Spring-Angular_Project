...
import { TimeSlot } from '../../../models/class-routine';
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage class time slots and periods</p>
      </div>
    ...
    <div class="card">
      <div class="table-header">
        <div class="search-box">
          ...
          <input type="text" [(ngModel)]="searchTerm" placeholder="Search time slots..." (input)="filterData()">
        </div>
          ...
          <tbody>
            @for (slot of filteredSlots; track slot.id) {
              <tr>
                ...
                <td>{{ slot.endTime }}</td>
                <td><span class="badge badge-info">{{ slot.slotType }}</span></td>
                ...
                <td>
                  <div class="actions">
                    <button class="btn-icon" (click)="openForm(slot)" title="Edit">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    <button class="btn-icon btn-danger" (click)="confirmDelete(slot)" title="Delete">
              ...
              <tr><td colspan="10" class="text-center text-muted">No time slots found</td></tr>
            }
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-overlay" (click)="closeForm()">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h3>{{ editingItem ? 'Edit' : 'New' }} Time Slot</h3>
          ...
          <form (ngSubmit)="save()">
            <div class="form-grid">
              <div class="form-group">
                <label>Name *</label>
                <input type="text" [(ngModel)]="form.name" name="name" required class="form-control" placeholder="e.g. Period 1">
              </div>
              <div class="form-group">
                <label>Code *</label>
                <input type="text" [(ngModel)]="form.code" name="code" required class="form-control" placeholder="e.g. P1">
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <select [(ngModel)]="form.slotType" name="slotType" required class="form-control">
              ...
              <div class="form-group">
                ...
                <input type="number" [(ngModel)]="form.durationMinutes" name="durationMinutes" required class="form-control">
              </div>
              <div class="form-group">
                <label>Sort Order *</label>
                <input type="number" [(ngModel)]="form.sortOrder" name="sortOrder" required class="form-control">
              </div>
...
export class TimeSlotComponent implements OnInit {
  ...
  saving = false;
  showForm = false;
  editingItem: TimeSlot | null = null;
  formError = '';
  showConfirm = false;
  confirmTitle = '';
  confirmMessage = '';
  deleteTarget: TimeSlot | null = null;
  searchTerm = '';
  filterType = '';

  form: Partial<TimeSlot> = {
    name: '',
    code: '',
    startTime: '',
    endTime: '',
  ...
  loadData() {
    this.loading = true;
    this.service.getTimeslots().subscribe({
      next: (data) => { this.slots = data; this.filterData(); this.loading = false; },
      error: () => { this.loading = false; this.toastService.error('Failed to load time slots'); }
    });
  }

  filterData() {
    this.filteredSlots = this.slots.filter(s => {
      const matchSearch = !this.searchTerm || s.name.toLowerCase().includes(this.searchTerm.toLowerCase()) || s.code.toLowerCase().includes(this.searchTerm.toLowerCase());
      const matchType = !this.filterType || s.slotType === this.filterType;
      return matchSearch && matchType;
    });
  }

  openForm(item?: TimeSlot) {
    this.editingItem = item ? { ...item } : null;
    this.form = item ? { ...item } : {
      name: '', code: '', startTime: '', endTime: '', slotType: '',
      durationMinutes: 45, sortOrder: 1, isActive: true, remarks: ''
    };
    this.formError = '';
    this.showForm = true;
  }
  ...
  save() {
    ...
    if (this.editingItem?.id) {
      this.service.updateTimeSlot(this.editingItem.id, this.form as TimeSlot).subscribe({ next: () => handleSuccess('Time slot updated'), error: handleError });
    } else {
      this.service.createTimeSlot(this.form as TimeSlot).subscribe({ next: () => handleSuccess('Time slot created'), error: handleError });
    }
  }

  confirmDelete(item: TimeSlot) {
    this.deleteTarget = item;
    this.confirmTitle = 'Delete Time Slot';
  ...
  executeDelete() {
    ...
    if (this.deleteTarget?.id) {
      this.service.deleteTimeSlot(this.deleteTarget.id).subscribe({
        next: () => { this.loadData(); this.toastService.success('Time slot deleted'); },
        error: () => this.toastService.error('Failed to delete time slot')
      });
    }
