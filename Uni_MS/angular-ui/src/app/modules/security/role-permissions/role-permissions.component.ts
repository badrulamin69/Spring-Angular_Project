  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage role permission assignments</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Role Permission' : 'Add New Role Permission'"
        [saving]="saving"
...
export class RolePermissionsComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'roleId', label: 'Role ID', type: 'number', required: true, placeholder: 'Role ID' },
    { key: 'permissionId', label: 'Permission ID', type: 'number', required: true, placeholder: 'Permission ID' }
  ];
  ...
  constructor(private service: RolePermissionService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load role permissions'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Role permission updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Role permission created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Role Permission';
    this.confirmMessage = `Are you sure you want to delete this role permission? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Role Permissions';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected role permissions? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} role permissions deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Role permission deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete role permission')
      });
    }
