...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>User Roles</h2>
        <p class="page-sub">Manage user role assignments</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit User Role' : 'Add New User Role'"
        [saving]="saving"
...
export class UserRolesComponent implements OnInit {
  ...
  constructor(private service: UserRoleService, private toastService: ToastService) {}
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('User role updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('User role created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete User Role';
    this.confirmMessage = `Are you sure you want to delete this user role? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete User Roles';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected user roles? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} user roles deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('User role deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete user role')
      });
    }
