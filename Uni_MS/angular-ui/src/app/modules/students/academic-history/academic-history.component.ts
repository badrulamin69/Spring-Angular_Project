  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">View student academic history and enrollment records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Academic History' : 'Add New Academic History'"
        [saving]="saving"
...
export class AcademicHistoryComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'status', label: 'Status', type: 'select', options: [{ label: 'Completed', value: 'COMPLETED' }, { label: 'In Progress', value: 'IN_PROGRESS' }, { label: 'Failed', value: 'FAILED' }] },
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load academic history'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    else { this.service.save(data).subscribe({ next: () => handleSuccess('Record created'), error: handleError }); }
  }

  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Record'; this.confirmMessage = 'Are you sure you want to delete this record?'; this.showConfirm = true; }
