...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Alumni</h2>
        <p class="page-sub">Manage alumni records and networking</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Alumni' : 'Add New Alumni'"
        [saving]="saving"
...
export class AlumniComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'isAvailableForMentoring', label: 'Available for Mentoring', type: 'select', options: [{ label: 'Yes', value: true }, { label: 'No', value: false }] },
    { key: 'isAvailableForRecruitment', label: 'Available for Recruitment', type: 'select', options: [{ label: 'Yes', value: true }, { label: 'No', value: false }] }
  ];
  showForm = false; editingItem: any = null; formError = '';
  showConfirm = false; confirmTitle = ''; confirmMessage = ''; deleteTarget: any = null;

  constructor(private service: AlumniService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load alumni'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) { this.service.update(this.editingItem.id, data).subscribe({ next: () => handleSuccess('Alumni updated'), error: handleError }); }
    else { this.service.create(data).subscribe({ next: () => handleSuccess('Alumni created'), error: handleError }); }
  }

  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Alumni'; this.confirmMessage = 'Are you sure you want to delete this alumni record?'; this.showConfirm = true; }
  confirmBulkDelete(items: any[]) { this.deleteTarget = items; this.confirmTitle = 'Bulk Delete'; this.confirmMessage = `Delete ${items.length} selected alumni?`; this.showConfirm = true; }
  executeDelete() {
    this.showConfirm = false;
    if (Array.isArray(this.deleteTarget)) {
      const ids = this.deleteTarget.map((i: any) => i.id);
      let completed = 0;
      ids.forEach((id: number) => { this.service.delete(id).subscribe({ next: () => { completed++; if (completed === ids.length) { this.loadData(); this.toastService.success(`${ids.length} alumni deleted`); } }, error: () => this.toastService.error('Failed to delete') }); });
    } else if (this.deleteTarget?.id) { this.service.delete(this.deleteTarget.id).subscribe({ next: () => { this.loadData(); this.toastService.success('Alumni deleted'); }, error: () => this.toastService.error('Failed to delete') }); }
