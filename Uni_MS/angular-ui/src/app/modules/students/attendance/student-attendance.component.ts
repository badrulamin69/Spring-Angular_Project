  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage student attendance records</p>
      </div>
...
export class StudentAttendanceComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'attendanceDate', label: 'Attendance Date', type: 'date', required: true },
    { key: 'status', label: 'Status', type: 'select', options: [{ label: 'Present', value: 'PRESENT' }, { label: 'Absent', value: 'ABSENT' }, { label: 'Late', value: 'LATE' }, { label: 'Excused', value: 'EXCUSED' }] },
    { key: 'remarks', label: 'Remarks', type: 'textarea' }
  ];
  showForm = false; editingItem: any = null; formError = '';
  showConfirm = false; confirmTitle = ''; confirmMessage = ''; deleteTarget: any = null;

  constructor(private service: StudentAttendanceService, private toastService: ToastService) {}
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) { this.service.update(this.editingItem.id, data).subscribe({ next: () => handleSuccess('Attendance updated'), error: handleError }); }
    else { this.service.create(data).subscribe({ next: () => handleSuccess('Attendance created'), error: handleError }); }
  }

  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Attendance'; this.confirmMessage = 'Are you sure you want to delete this attendance record?'; this.showConfirm = true; }
