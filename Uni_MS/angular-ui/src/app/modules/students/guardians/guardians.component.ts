...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Guardians</h2>
        <p class="page-sub">Manage guardian records</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Guardian' : 'Add New Guardian'"
        [saving]="saving"
...
export class GuardiansComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'relationship', label: 'Relationship', type: 'text', required: true, placeholder: 'Parent/Guardian' },
    { key: 'email', label: 'Email', type: 'email', placeholder: 'guardian@example.com' },
    { key: 'phone', label: 'Phone', type: 'text', required: true, placeholder: '+1 234 567 890' },
  ...
  constructor(private service: GuardianService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load guardians'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Guardian updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Guardian created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Guardian';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Guardians';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected guardians? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} guardians deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Guardian deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete guardian')
      });
    }
