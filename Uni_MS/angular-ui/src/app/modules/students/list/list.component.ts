...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Students</h2>
        <p class="page-sub">Manage student records and profiles</p>
      </div>
...
export class StudentsListComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'uniqueCode', label: 'Student Code', sortable: true },
    { key: 'firstName', label: 'First Name', sortable: true, type: 'text', required: true, placeholder: 'Enter first name' },
  ...
  constructor(private service: StudentService, private toastService: ToastService) {}
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Student updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Student created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Students';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected students? This action cannot be undone.`;
    this.showConfirm = true;
  }
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} students deleted`);
            }
          },
