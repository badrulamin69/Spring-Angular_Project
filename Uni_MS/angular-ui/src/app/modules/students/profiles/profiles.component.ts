  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage student profile details</p>
      </div>
...
export class ProfilesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'bloodGroup', label: 'Blood Group', type: 'text', placeholder: 'A+/B-/O+' },
    { key: 'emergencyContact', label: 'Emergency', type: 'text', placeholder: 'Emergency contact' }
  ];
  ...
  constructor(private service: StudentProfileService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load student profiles'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Student profile updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Student profile created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Student Profile';
    this.confirmMessage = `Are you sure you want to delete this student profile? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Student Profiles';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected student profiles? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} student profiles deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Student profile deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete student profile')
      });
    }
