  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage student semester registrations</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Semester Registration' : 'Add New Semester Registration'"
        [saving]="saving"
...
export class SemesterRegistrationComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'status', label: 'Status', type: 'select', options: [{ label: 'Registered', value: 'REGISTERED' }, { label: 'Cancelled', value: 'CANCELLED' }, { label: 'Pending', value: 'PENDING' }] },
    { key: 'remarks', label: 'Remarks', type: 'textarea' }
  ];
  showForm = false; editingItem: any = null; formError = '';
  showConfirm = false; confirmTitle = ''; confirmMessage = ''; deleteTarget: any = null;

  constructor(private service: SemesterRegistrationService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load semester registrations'); }
    });
  }
  ...
  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Registration'; this.confirmMessage = 'Are you sure you want to delete this semester registration?'; this.showConfirm = true; }
