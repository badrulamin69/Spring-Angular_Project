  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage student promotions between semesters and batches</p>
      </div>
...
export class StudentPromotionComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'fromSemesterId', label: 'From Semester ID', type: 'number' },
    { key: 'toSemesterId', label: 'To Semester ID', type: 'number' },
    { key: 'fromBatchId', label: 'From Batch ID', type: 'number' },
    { key: 'toBatchId', label: 'To Batch ID', type: 'number' },
    { key: 'status', label: 'Status', type: 'select', options: [{ label: 'Promoted', value: 'PROMOTED' }, { label: 'Detained', value: 'DETAINED' }, { label: 'Pending', value: 'PENDING' }] },
    { key: 'remarks', label: 'Remarks', type: 'textarea' }
  ];
  showForm = false; editingItem: any = null; formError = '';
  showConfirm = false; confirmTitle = ''; confirmMessage = ''; deleteTarget: any = null;

  constructor(private service: StudentPromotionService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load promotions'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) { this.service.update(this.editingItem.id, data).subscribe({ next: () => handleSuccess('Promotion updated'), error: handleError }); }
    else { this.service.create(data).subscribe({ next: () => handleSuccess('Promotion created'), error: handleError }); }
  }

  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Promotion'; this.confirmMessage = 'Are you sure you want to delete this promotion record?'; this.showConfirm = true; }
  confirmBulkDelete(items: any[]) { this.deleteTarget = items; this.confirmTitle = 'Bulk Delete'; this.confirmMessage = `Delete ${items.length} selected promotions?`; this.showConfirm = true; }
  executeDelete() {
    this.showConfirm = false;
    if (Array.isArray(this.deleteTarget)) {
      const ids = this.deleteTarget.map((i: any) => i.id);
      let completed = 0;
      ids.forEach((id: number) => { this.service.delete(id).subscribe({ next: () => { completed++; if (completed === ids.length) { this.loadData(); this.toastService.success(`${ids.length} promotions deleted`); } }, error: () => this.toastService.error('Failed to delete') }); });
    } else if (this.deleteTarget?.id) { this.service.delete(this.deleteTarget.id).subscribe({ next: () => { this.loadData(); this.toastService.success('Promotion deleted'); }, error: () => this.toastService.error('Failed to delete') }); }
