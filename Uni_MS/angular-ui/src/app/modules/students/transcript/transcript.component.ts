...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Transcripts</h2>
        <p class="page-sub">Manage student transcripts</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Transcript' : 'Add New Transcript'"
        [saving]="saving"
...
export class TranscriptComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'transcriptNumber', label: 'Transcript Number', type: 'text' },
    { key: 'studentId', label: 'Student ID', sortable: true, type: 'number', required: true },
    { key: 'programId', label: 'Program ID', type: 'number' },
    { key: 'semesterId', label: 'Semester ID', type: 'number' },
    { key: 'status', label: 'Status', type: 'select', options: [{ label: 'Draft', value: 'DRAFT' }, { label: 'Issued', value: 'ISSUED' }, { label: 'Revoked', value: 'REVOKED' }] },
  ...
  constructor(private service: TranscriptService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load transcripts'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) { this.service.update(this.editingItem.id, data).subscribe({ next: () => handleSuccess('Transcript updated'), error: handleError }); }
    else { this.service.create(data).subscribe({ next: () => handleSuccess('Transcript created'), error: handleError }); }
  }

  confirmDelete(item: any) { this.deleteTarget = item; this.confirmTitle = 'Delete Transcript'; this.confirmMessage = 'Are you sure you want to delete this transcript?'; this.showConfirm = true; }
  confirmBulkDelete(items: any[]) { this.deleteTarget = items; this.confirmTitle = 'Bulk Delete'; this.confirmMessage = `Delete ${items.length} selected transcripts?`; this.showConfirm = true; }
  executeDelete() {
    this.showConfirm = false;
    if (Array.isArray(this.deleteTarget)) {
      const ids = this.deleteTarget.map((i: any) => i.id);
      let completed = 0;
      ids.forEach((id: number) => { this.service.delete(id).subscribe({ next: () => { completed++; if (completed === ids.length) { this.loadData(); this.toastService.success(`${ids.length} transcripts deleted`); } }, error: () => this.toastService.error('Failed to delete') }); });
    } else if (this.deleteTarget?.id) { this.service.delete(this.deleteTarget.id).subscribe({ next: () => { this.loadData(); this.toastService.success('Transcript deleted'); }, error: () => this.toastService.error('Failed to delete') }); }
