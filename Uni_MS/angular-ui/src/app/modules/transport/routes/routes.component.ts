  ...
  template: `
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Manage transport routes</p>
      </div>
    ...
    @if (showForm) {
      <app-dynamic-form
        ...
        [title]="editingItem ? 'Edit Transport Route' : 'Add New Transport Route'"
        [saving]="saving"
...
export class RoutesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'name', label: 'Name', sortable: true, type: 'text', required: true, placeholder: 'Route name' },
    { key: 'routeCode', label: 'Code', sortable: true, type: 'text', required: true, placeholder: 'RT001' },
    { key: 'startPoint', label: 'Start', sortable: true, type: 'text', required: true, placeholder: 'Start point' },
    { key: 'endPoint', label: 'End', sortable: true, type: 'text', required: true, placeholder: 'End point' },
    { key: 'distanceKm', label: 'Distance km', type: 'number', placeholder: '25.5' },
    { key: 'fare', label: 'Fare', sortable: true, type: 'number', placeholder: '500.00' }
  ];
  ...
  constructor(private service: RouteService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params, this.searchTerm).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load transport routes'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Transport route updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Transport route created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Transport Route';
  ...
  confirmBulkDelete(items: any[]) {
    ...
    this.confirmTitle = 'Bulk Delete Transport Routes';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected transport routes? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} transport routes deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        next: () => {
          this.loadData();
          this.toastService.success('Transport route deleted successfully');
        },
        error: () => this.toastService.error('Failed to delete transport route')
      });
    }
