...
@Component({
  ...
  template: `
    ...
    <div class="page-header">
      <div>
        <h2>Vehicles</h2>
        <p class="page-sub">Manage transport vehicles</p>
      </div>
...
export class VehiclesComponent implements OnInit {
  ...
  columns: TableColumn[] = [
    ...
    { key: 'vehicleNumber', label: 'Vehicle No.', sortable: true, type: 'text', required: true, placeholder: 'VH001' },
    { key: 'vehicleType', label: 'Type', sortable: true, type: 'text', required: true, placeholder: 'Bus/Van' },
    { key: 'capacity', label: 'Capacity', sortable: true, type: 'number', required: true, placeholder: '40' },
    { key: 'driverName', label: 'Driver', type: 'text', placeholder: 'Driver name' },
    { key: 'driverPhone', label: 'Driver Phone', type: 'text', placeholder: '+1 234 567 890' }
  ];
  ...
  constructor(private service: VehicleService, private toastService: ToastService) {}
  ...
  loadData() {
    ...
    this.service.findAll(this.params).subscribe({
      ...
      error: () => { this.loading = false; this.toastService.error('Failed to load vehicles'); }
    });
  }
  ...
  saveItem(data: any) {
    ...
    if (this.editingItem?.id) {
      this.service.update(this.editingItem.id, data).subscribe({
        next: () => handleSuccess('Vehicle updated successfully'),
        error: handleError
      });
    } else {
      this.service.save(data).subscribe({
        next: () => handleSuccess('Vehicle created successfully'),
        error: handleError
      });
    }
  }
  ...
  confirmDelete(item: any) {
    ...
    this.confirmTitle = 'Delete Vehicle';
    this.confirmMessage = `Are you sure you want to delete "${item.vehicleNumber}"? This action cannot be undone.`;
    this.showConfirm = true;
  }

  confirmBulkDelete(items: any[]) {
    this.deleteTarget = items;
    this.confirmTitle = 'Bulk Delete Vehicles';
    this.confirmMessage = `Are you sure you want to delete ${items.length} selected vehicles? This action cannot be undone.`;
    this.showConfirm = true;
  }
  ...
  executeDelete() {
    ...
    if (Array.isArray(this.deleteTarget)) {
        ...
        this.service.delete(id).subscribe({
          next: () => {
            ...
            if (completed === ids.length) {
              this.loadData();
              this.toastService.success(`${ids.length} vehicles deleted`);
            }
          },
      ...
      this.service.delete(this.deleteTarget.id).subscribe({
        ...
        error: () => this.toastService.error('Failed to delete vehicle')
      });
    }
