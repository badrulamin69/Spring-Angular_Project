...
public class ProgramController {
    ...
    @GetMapping
    public ResponseEntity<PagedResponse<Program>> findAll(
    ...
    @PostMapping
    public ResponseEntity<Program> save(@Valid @RequestBody Program program) {
        return ResponseEntity.ok(programService.save(program));
    }

    @PreAuthorize("hasAuthority('ACADEMIC_MANAGE')")
    @PutMapping("/{id}")
    public ResponseEntity<Program> update(@PathVariable Long id, @Valid @RequestBody Program program) {
