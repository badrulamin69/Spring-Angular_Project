...
public class WorkflowController {
    ...
    @GetMapping
    public ResponseEntity<PagedResponse<Workflow>> findAll(
    ...
    @PreAuthorize("hasAuthority('SETTINGS_MANAGE')")
    @PostMapping
    public ResponseEntity<Workflow> save(@Valid @RequestBody Workflow workflow) {
        return ResponseEntity.ok(workflowService.save(workflow));
    }

    @PreAuthorize("hasAuthority('SETTINGS_MANAGE')")
    @PutMapping("/{id}")
    public ResponseEntity<Workflow> update(@PathVariable Long id, @Valid @RequestBody Workflow workflow) {
    ...
    @PreAuthorize("hasAuthority('SETTINGS_MANAGE')")
    @GetMapping("/{id}/steps")
    public ResponseEntity<List<WorkflowStep>> getSteps(@PathVariable Long id) {
        return ResponseEntity.ok(workflowService.getSteps(id));
    }

    @PreAuthorize("hasAuthority('SETTINGS_MANAGE')")
    @PostMapping("/{id}/steps")
    public ResponseEntity<WorkflowStep> addStep(@PathVariable Long id, @Valid @RequestBody WorkflowStep step) {
        return ResponseEntity.ok(workflowService.addStep(id, step));
    }

    @PreAuthorize("hasAuthority('SETTINGS_MANAGE')")
    @PutMapping("/steps/{stepId}")
    public ResponseEntity<WorkflowStep> updateStep(@PathVariable Long stepId, @Valid @RequestBody WorkflowStep step) {
    ...
    public ResponseEntity<List<WorkflowApproval>> getApprovals(
        ...
        return ResponseEntity.ok(workflowService.getApprovals(entityType, entityId));
    }
